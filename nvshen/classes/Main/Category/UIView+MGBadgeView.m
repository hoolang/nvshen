...
static float const kMGBadgeViewInnerSpaceFromBorder = 5.0;// 7.0
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        _outlineWidth = 0.0;//2.0
        _minDiameter = 16.0;//25
...
- (void)mg_updateBadgeViewPosition {
    ...
    switch (position) {
        ...
        case MGBadgePositionCenterRight: {
            self.center = CGPointMake(superviewFrame.size.width + _horizontalOffset - 5, superviewFrame.size.height / 2 + _verticalOffset);
            break;
        }
        case MGBadgePositionTopRight: {
            self.center = CGPointMake(superviewFrame.size.width + _horizontalOffset - 10, superviewFrame.origin.y + 12);
            break;
        }
